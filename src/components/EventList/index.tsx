      ...
      {eventsForCurrentPage.map((iEvent) => (
        ...
        >
          ...
          <div className="flex flex-col md:flex-row h-full  w-full sm:p-0 p-1">
            {iEvent.imageURL ? (
              <img src={iEvent.imageURL} className="md:w-72 w-full h-auto p-2 mr-3 rounded-xl" alt="test" />
            ) : (
              <Box className="md:w-80 h-92 w-full items-center p-16 mr-5 ml-2 rounded-lg flex flex-col text-center" bg={eventDateBG} color={eventDateColor}>
            ...
            <div className="ml-3 w-8/12">
              ...
              <div className="flex flex-row w-12/12 w-full sm:p-0  md:items-center">
                ...
                <div className="md:w-6/12 w-full sm:p-0 ml-1 md:text-center mb-5">
                  <Badge fontSize="0.8em" colorScheme={getStatusColor(getStatus(statuses.Event, iEvent.StatusId).tag || '')}>
            ...
            <Box className="w-4/12 flex justify-end font-bold my-3 md:flex hidden">{moment(iEvent.startTime).format('ddd • MMMM DD')}</Box>
          </div>
        </Box>
      ))}
      {/* Pagination buttons */}
