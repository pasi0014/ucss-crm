...
const GuestCard: React.FC<IGuestCard> = ({
  statuses,
  clientList,
  enableDelete = true,
  onDelete,
  ...
  return (
    <Box
      className="flex flex-col rounded-xl shadow-md p-3"
    ...
    >
      ...
      <div className="flex sm:flex-row flex-col justify-space-between">
        ...
        <div className="flex flex-row sm:w-4/12 w-full sm:my-auto my-5 h-full items-center align-baseline text-base">
          ...
          <span>
            {clientList.Price?.name} |{' '}
            {clientList.Price?.ticketType === 'free' ? 'Free' : `$${clientList.Price?.amount}`}
          </span>
        </div>
        <div className="flex flex-col justify-end mx-auto p-1">
          ...
          <span className="text-center text-xs mt-2">
            ReservationCode: {clientList.reservationCode}
          </span>
        </div>
      </div>
      {/* Tickets info */}
      <div className="flex sm:flex-row flex-col">
        <div className="flex flex-row justify-start p-3 sm:w-8/12 w-full">
          {clientList.StatusId !== statuses.ClientList.REDEEMED && (
            <Button
              className="mt-2 sm:w-6/12 w-full  mr-2"
              colorScheme="green"
              size={{ base: 'sm' }}
              onClick={() => onCheckIn(clientList)}
            >
        ...
        <div className="flex flex-row justify-end p-3 sm:w-8/12 w-full">
          <Button
            ...
            colorScheme="orange"
            size={{ base: 'sm' }}
            onClick={() => onEdit && onEdit(clientList)}
          >
          ...
          {enableDelete && (
            <Button
              ...
              colorScheme="red"
              size={{ base: 'sm' }}
              onClick={() => handleDelete(clientList)}
            >
